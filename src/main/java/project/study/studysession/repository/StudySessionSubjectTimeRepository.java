...
public interface StudySessionSubjectTimeRepository extends JpaRepository<StudySessionSubjectTime, Long> {
    ...
    @Query("""
            select new project.study.studysession.dto.SubjectTimeSum(t.taskId, sum(t.studySec), sum(t.focusSec))
            from StudySessionSubjectTime t
            where t.taskId in :taskIds
            group by t.taskId""")
    List<SubjectTimeSum> sumByTaskIds(@Param("taskIds") Collection<Long> taskIds);
}
