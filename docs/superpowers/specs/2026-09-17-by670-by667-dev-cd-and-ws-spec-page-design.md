...
# git은 소유자(ubuntu)로 — SSM은 root로 실행되므로 .git 소유권이 꼬이지 않게 한다
sudo -u ubuntu -H git checkout dev
sudo -u ubuntu -H git pull --ff-only origin dev
# docker는 데몬 기반이라 root로 실행해도 사용자가 손으로 띄운 것과 같은 프로젝트(app-dev)를 갱신한다
docker compose up -d --build
docker image prune -f
for _ in $(seq 1 30); do
  if curl -sf http://localhost:8080/actuator/health >/dev/null; then echo "healthy"; exit 0; fi
  sleep 2
done
...
| `.env` 누락 | 새 컨테이너 기동 실패 → health 실패 | BY-642 절차로 `.env` 반영 |
| 동시 push | 큐잉 후 순서대로 | 없음 |
| SSM 에이전트 오프라인 | send-command 자체가 `InvalidInstanceId` | 런북 4 |
