...
- **확정은 기존 create 재사용**: 검증·자정 분할·statDate 계산·멱등을 그대로
  쓴다. 하트비트 검증도 같은 코드(validateAndBuildSessions)를 호출해 결과만 버리므로
  draft는 항상 확정 가능하다. 길이 무관 전부 확정한다(ADR-0009).
- **확정 실패 시 폐기는 영구 실패로 한정**: 검증 실패·유저 없음·역직렬화
  불가만 재시도해도 영원히 실패하는 영구 실패로 보고 draft를 폐기한다. 그 외
  (커넥션 끊김·락 타임아웃 등 일시 추정 오류)는 draft를 보존해 다음 틱에
  자연 재시도되게 한다 — 모든 예외에서 폐기하면 이 기능이 막으려던 세션
  유실을 서버가 대신 일으키게 된다. 확정이 멱등 수렴이라 재시도는 안전하다.
